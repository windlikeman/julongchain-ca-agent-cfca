...
class ReenrollmentComms {
     ...
     */
    EnrollmentResponseNet request(ReenrollmentRequestNet enrollmentRequest, String auth) throws CommandException {
        final String baseUrl = clientConfig.getUrl();
        if (StringUtils.isBlank(baseUrl)) {
            throw new CommandException(CommandException.REASON_CODE_REENROLL_COMMAND_COMMS_FAILED, "baseurl is empty");
        }
    ...
    private EnrollmentResponseNet syncHandle(Call<EnrollmentResponseNet> call) throws CommandException {
        try {
            ...
            if (!decodedResponse.isSuccess()) {
                ...
                throw new CommandException(CommandException.REASON_CODE_REENROLL_COMMAND_COMMS_FAILED, "server internal error : " + errorMessage);
            }
